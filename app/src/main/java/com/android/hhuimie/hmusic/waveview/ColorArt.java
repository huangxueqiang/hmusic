...
public class ColorArt {
	...
	private int findEdgeColor() {
		...
		double colorThresholdMinimumPercentage = 0.01;
		int randomColorThreshold = (int) (height * colorThresholdMinimumPercentage);
		...
		while (sortedColorIterator.hasNext()) {
			...
			double edgeColorDiscardThreshold = 0.3;
			if (edgeColorRatio <= edgeColorDiscardThreshold) {
				break;
			}
	...
	private void findTextColors(HashBag<Integer> colors) {
		...
		while (iterator.hasNext()) {
			...
			float minimumSaturationThreshold = 0.15f;
			currentColor = colorWithMinimumSaturation(currentColor, minimumSaturationThreshold);
	...
	private boolean isDistinctColor(int colorA, int colorB) {
		...
		double threshold = .25;
		return (Math.abs(r - r1) > threshold || Math.abs(g - g1) > threshold || Math.abs(b - b1) > threshold || Math.abs(a - a1) > threshold) && !(Math.abs(r - g) < .03 && Math.abs(r - b) < .03 && (Math.abs(r1 - g1) < .03 && Math.abs(r1 - b1) < .03));
	}
